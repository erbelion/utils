...
Warning: This package is experimental, updates may be critical.

## Installation
**yarn**
```
yarn add @erbelion/utils --dev
```
**npm**
```
npm i @erbelion/utils --save-dev
```
...
### setUrlParams
check browser example [here](https://codesandbox.io/s/utils-example-ctxod4?file=/src/App.vue)
```
import { setUrlParams } from '@erbelion/utils'

setUrlParams({ b: 2 }, 'https://github.com/erbelion/utils?a=1') // { a: 1, b: 2 }
setUrlParams({ b: 2 }, { a: 1 }) // { a: 1, b: 2 }

// if no second arg is given, then browser url (window.location.href) is updated without reload
setUrlParams({ b: 2 }) // updates current window url to https://github.com/erbelion/utils?b=2
```